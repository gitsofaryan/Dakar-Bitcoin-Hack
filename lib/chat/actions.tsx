...
import { BitcoinRecommendation } from '@/components/stocks/bitcoin-recommendation'
...
When users ask about Bitcoin trading recommendations, buy/sell/hold advice, or whether to trade Bitcoin, use the analyzeBitcoin tool. This tool uses advanced AI to analyze Bitcoin market trends and provide actionable recommendations.
...
Assistant (you): { "tool_call": { "id": "pending", "type": "function", "function": { "name": "analyzeBitcoin" }, "parameters": {} } } 
    `,
      messages: [
        ...aiState.get().messages.map((message: any) => ({
          role: message.role,
      ...
      tools: {
        ...
        analyzeBitcoin: {
          description: `Analyze Bitcoin price trends and provide BUY, SELL, or HOLD recommendation based on technical indicators including RSI, moving averages, and market sentiment. Use this when the user asks about Bitcoin trading advice or whether to buy, sell, or hold Bitcoin.`,
          parameters: z.object({}),
          generate: async function* ({ }) {
            yield (
              <BotCard>
                <div className="animate-pulse">Analyzing Bitcoin market trends...</div>
              </BotCard>
            )
            ...
            try {
              ...
              const chatCompletion = await groqClient.chat.completions.create({
                messages: [
                  {
                    role: 'system',
                    content: `You are an expert cryptocurrency analyst. Analyze Bitcoin and provide a trading recommendation in JSON format.

CRITICAL: Your response must be ONLY valid JSON, nothing else. No markdown, no explanation, just pure JSON.

Return this exact structure:
{
  "recommendation": "BUY" or "SELL" or "HOLD",
  "confidence": number between 0-100,
  "reasoning": "Clear, professional analysis covering: current trend, RSI condition, key support/resistance levels, and recommendation rationale. Use bullet points with • for better readability.",
  "price": current Bitcoin price in USD as a number,
  "rsi": current RSI value as a number
}

Base your analysis on:
- Current price action and trend direction
- RSI levels (>70 overbought, <30 oversold, 30-70 neutral)
- Recent volatility and volume patterns
- Key support and resistance levels
- Overall market sentiment

Make the reasoning clear, actionable, and professional. Keep it concise (3-5 key points).`
                  },
                  {
                    role: 'user',
              ...
              try {
                ...
                const recommendation = analysisText.match(/(?:recommendation|action)["']?\s*:\s*["']?(BUY|SELL|HOLD)/i)?.[1] as 'BUY' | 'SELL' | 'HOLD' || 'HOLD'
                const confidence = parseInt(analysisText.match(/(?:confidence|score)["']?\s*:\s*["']?(\d+)/i)?.[1] || '50')
                const reasoning = analysisText.replace(/\*\*/g, '').substring(0, 500)
              ...
              return (
                <BotCard>
                  <div className="text-red-500">
                    Error analyzing Bitcoin: {error.message}
                  </div>
                </BotCard>
              )
            }
